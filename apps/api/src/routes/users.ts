...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.post('/invite', { preHandler: requirePermission('users:write') }, async (req, reply) => {
    ...
    // Only return the password in the response if the caller supplied it explicitly.
    // Auto-generated passwords are never echoed back — share via secure channel.
    const responsePayload: any = { success: true, data: user }
    if (!isAutoGenerated) responsePayload.tempPassword = tempPassword
